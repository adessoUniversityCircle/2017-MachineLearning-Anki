...
/**
 * Requests the vehicle to change its light pattern according to the given config.
 * 
 * (not yet implemented)
 * 
  ...
  public static class LightConfig {
    ...
    public static LightConfig fromBytes(byte[] rawConfig) {
      return new LightConfig(
          LightChannel.fromByte(rawConfig[0]),
          LightEffect.fromByte(rawConfig[1]),
          Byte.toUnsignedInt(rawConfig[2]),
          Byte.toUnsignedInt(rawConfig[3]),
